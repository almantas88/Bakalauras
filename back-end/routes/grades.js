...
router.post("/newGrade", async (req, res) => {
  try {
    ...
    await newGrade.save();
    res.status(200).json({ msg: "Klasė sukurta!" });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
});

router.delete("/deleteGrade", async (req, res) => {
  try {
    const existingGrade = await Grade.findOne({ _id: req.body._id });
    if (!existingGrade)
      return res.status(400).json({ msg: "Nera tokios klasės." });

      await Grade.deleteOne({ _id: req.body._id });
